...
d3.select('#select-key').on('change', function(a) {
    ...
    if ( (currentKey == 'n') | (currentKey == 's') ) {
        legendXAxis.tickFormat( (d) => formatInteger(d) );
    } else {
        legendXAxis.tickFormat( (d) => formatNumber(d) );
    }
    updateMapColors();
});
...
// We prepare a number format which will always return 4 decimal places.
...
function calcular_maximos(data) {
     ...
     * Args:
     *     data: tiene la estructura de dataById.
     *     {
     *         UN_ID: {a: valor_a, alfa_fisher: vañor_alfa, b: valor_b, …},
     *         OTRO_ID: {a: valor_a, alfa_fisher: vañor_alfa, b: valor_b, …}
     *         ...
     *     }
    **/
    var maxs = {};
...
d3.json("/static/capas/geojson/ign-departamentos.geojson", function(error, features) {
    ...
    d3.json('/indicadores/departamentos', function(response){
        ...
        dataById = d3.nest()
            .key( (d) => parseInt(d.departamento_id) )
        ...
        mapFeatures.selectAll('path')
            ...
            .attr('d', path)
            // Le agrego una clase para despues poder hacer el filtrado
        ...
        // Una vez que tengamos la info de departamentos, podemos cargar la lista del filtro:
        loadFilterList()
...
var ACTIVE_FEATURE = d3.select(null);
...
function showDetails(feature) {
    //do_zoom_to(this, feature);
    ACTIVE_FEATURE.classed("active", false);
    ACTIVE_FEATURE = d3.select(this).classed("active", true);
    
    // Dejar de seleccionar los filtrados tambien:
    ...
    data_item.n = parseInt(data_item.n);
    data_item.s = parseInt(data_item.s);
    data_item.ins = parseFloat(parseFloat(data_item.ins).toFixed(4));
    data_item.consanguinidad = parseFloat(parseFloat(data_item.consanguinidad).toFixed(4));
    data_item.alfa_fisher = parseFloat(parseFloat(data_item.alfa_fisher).toFixed(2));
    data_item.a = parseFloat(parseFloat(data_item.a).toFixed(4));
    data_item.b = parseFloat(parseFloat(data_item.b).toFixed(4));

    if ( ( data_item.provincia_nombre == 'Buenos Aires' ) || ( data_item.provincia_nombre == 'Capital Federal') ){
        data_item.link_detalle = `/detalle/departamento/00${data_item.departamento_id}`;
    }else{
        data_item.link_detalle = `/detalle/departamento/0${data_item.departamento_id}`;
    }
    // Render the Mustache template with the data object and put the
    // resulting HTML output in the details container.
    var customTags = [ '<%', '%>' ];
    var detailsHtml = Mustache.render(template, data_item, {}, customTags);
...
function showTooltip(f) {
  ...
  var top = mouse[1] + 115;
    ...
    var valor;
    if ( (currentKey == 'n') | (currentKey == 's') ) {
        valor = formatInteger(getValueOfData(d));
    } else {
        valor = formatNumber(getValueOfData(d));
    }
  ...
  tooltip.classed('hidden', false)
    ...
    //.html(d.name);
    .html(`${d.departamento_nombre} [${valor}]`);
}
...
function calculateScaleCenter(features) {
  ...
  //var bbox_path = [[-73.56659, -55.06352], [-53.63737, -21.78113]];
...
function getIdOfFeature(f) {
  //return parseInt(f.properties['id']);
  return parseInt(f.properties['in1']);
}
...
let filtro = {
    ...
    handle_change: (department_selected_code) => {
        d3.select(".filtered").classed("filtered", false);
        //department_selected_code = department_selected_code.length != 6 ? '0'+department_selected_code : department_selected_code
...
//filtro.handle_change("026077")

function connect_sidebar_buttons(){
...
connect_sidebar_buttons()

function onFilterListKeyUp() {
...
function createListItem(item_text, target) {
    var ul = document.getElementById("filter-item-list");
    ...
    li.addEventListener(
        "click",
        function(event){
            filtro.handle_change(target.toString().length == 5 ? '0'+target : '00'+target);
            event.preventDefault();  // evitamos salto...
        });
